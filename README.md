...
manager:
  ...
  qha: true                           # 开启 QHA 工作流
  bte: false                          # 开启 BTE 工作流
  plain_submit: false                 # true 时仅跟踪并提交已有 task.*，不生成下游任务
...
`plain_submit: true` 时，Manager 只递归扫描并跟踪已有的 `task.*` 目录，不自动生成 `opt`、`phonon`、`qha`、`bte` 任务，也不做后处理。

VASP 模式下：
- `task.*` 会读取 `incar.plain`
- 并按 `vasp_sets.plain` 选择输入集
- 如果没有配置 `incar.plain`，会回退到 `incar.phonon`，再回退到 `incar.opt`
...
│  3. 以数据库为主跟踪任务状态，运行态再结合 squeue 轻量校验      │
│  4. 后处理（声子、QHA、虚频检测）                             │
│  5. 数据库自动备份（每10分钟）                                │
└─────────────────────────────────────────────────────────────┘
                              │
...
# 任务管理
qflow cancel                   # 仅取消当前工作目录下 manager 和它提交的任务
qflow reset --running          # 按数据库记录重置 running，并清理对应 .running
qflow reset --failed           # 按数据库记录重置 failed，并清理对应失败标记
...
# 队列同步
qflow sync                     # 快速扫描并注册新任务到数据库，不逐项回写旧任务状态
...
说明：
- plain 模式只会把 `task.*` 放入提交队列
- `task_perfect`、`opt` 不会在 plain 模式下注册
- 每个子任务自己的提交状态和错误记录都保留在对应 `task.*` 目录内
...
5. **故障恢复**：
   - `qflow reset --running` / `qflow reset --failed` 会按数据库中的路径和状态重置
   - reset 同时会清理对应任务目录下的状态标记文件
   - 数据库损坏时可从 `tasks.db.backup` 恢复

6. **同步与状态**：
   - `qflow status` 默认直接读数据库统计，只对 running 任务做轻量校验
   - `qflow sync` 只负责快速扫描并注册新任务，老任务状态由 manager 轮询时同步
   - 快速扫描使用 `os.scandir()` 递归、正则分类和 SQLite 批量 `INSERT OR IGNORE`
...
```bash
qflow cancel        # 取消当前工作目录下的作业并清理对应 .running 标记
...
**Q: plain_submit 用哪套 INCAR / VASP set？**
```yaml
manager:
  plain_submit: true

vasp_sets:
  plain: matpes     # 或 mpstatic / mprelax
...
plain 模式下，`task.*` 会使用 `incar.plain` 和 `vasp_sets.plain`。
